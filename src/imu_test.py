...
def read(serial_port):
    ...
    if len(line) >= 2 :# valid data string will be longer than 9 characters  
        ...
        # read gyroscope data    
            ...
            q = re.findall(r"[-+]?\d*\.\d+|\d+", line)# extract imu data (decimal number) from the data string
            gyroscope = [gyr for gyr in q]
            print('Angular Velocity: ',gyroscope)

        # read magnetometer data    
        elif line[0] == "M":
            q = re.findall(r"[-+]?\d*\.\d+|\d+", line)# extract imu data (decimal number) from the data string
            magnetometer = [mag for mag in q]
    ...
    for imu_id in range(num_imus):
        ...
        if any(imu[imu_id]) == None:
            ...
            IMU.y = imu[imu_id][1]
            IMU.z = imu[imu_id][2]
            IMU.ax = float(acceleration[0])
            IMU.ay = float(acceleration[1])
            IMU.az = float(acceleration[2])
            IMU.gx = float(gyroscope[0])
            IMU.gy = float(gyroscope[1])
            IMU.gz = float(gyroscope[2])
            IMU.mx = float(magnetometer[0])
            IMU.my = float(magnetometer[1])
            IMU.mz = float(magnetometer[2])
...
if __name__ == '__main__':
    ...
    num_imus = 1#set number of inertial measurement units
    count = 0
    cap = [0]*num_sensors
    capacitance = [0]*num_sensors
    acceleration = [0]*3*num_imus
    gyroscope = [0]*3*num_imus
    magnetometer = [0]*3*num_imus
    orientation = [0]*3
