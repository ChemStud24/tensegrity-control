...
class Visualiser:
    ...
    def callback(self, msg):
    	# reconstruction
        self.rdata = [[node.x,node.y,node.z] for node in msg.reconstructed_nodes]
        # motion capture
        self.mdata = [[node.x,node.y,node.z] for node in msg.mocap_nodes]
        # raw imu direction
        self.idata = [np.array([imu.x,imu.y,imu.z])*L/2 for imu in msg.imus]
    ...
    def update_plot(self, frame):
        ...
        # motion capture
        data = np.array(self.mdata)
        if data.shape[0] > 0:
	        self.mscat._offsets3d = (data[:,0],data[:,1],data[:,2])
        return self.rscat


rospy.init_node('reconstruction_plotter')
vis = Visualiser()
sub = rospy.Subscriber('/reconstruction_msg', NodesStamped, vis.callback)
